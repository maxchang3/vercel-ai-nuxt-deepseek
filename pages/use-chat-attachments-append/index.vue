  ...
  <div class="flex flex-col w-full max-w-md py-24 mx-auto stretch">
    <div v-for="message in messages" :key="message.id" class="whitespace-pre-wrap">
      ...
      <div v-if="message.experimental_attachments" class="flex flex-row gap-2">
        <img v-for="(attachment, index) in message.experimental_attachments" :key="`${message.id}-${index}`" :src="attachment.url"
          class="rounded-md size-20" />
      </div>
    </div>

    <form @submit.prevent="submit" class="fixed bottom-0 flex flex-col max-w-md gap-2 mb-8">
      ...
      <input class="w-full p-2 border border-gray-300 rounded shadow-xl" v-model="input" placeholder="Say something..." />
    </form>
  </div>
</template>
