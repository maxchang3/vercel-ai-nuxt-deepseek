...
<template>
  <div class="flex flex-col w-full max-w-md py-24 mx-auto stretch">
    ...
    <div class="relative px-6 py-4 text-white bg-red-500 rounded-md" v-if="error">
      <span class="block sm:inline"> Error: {{ error?.toString() }} </span>
    </div>

    <!-- Render Assistant Messages -->
    <div class="whitespace-pre-wrap" v-for="(message, index) in messages" :key="index" :style="{ color: roleToColorMap[message.role] }">
    ...
    <div class="w-full h-8 max-w-md p-2 mb-8 bg-gray-300 rounded-lg dark:bg-gray-600 animate-pulse" v-if="status === 'in_progress'"></div>

    <!-- Render Assistant Message Input Form -->
    <form @submit.prevent="(e) => handleSubmit(e as any)">
      <input ref="inputRef" :disabled="status === 'in_progress'"
        class="fixed w-full max-w-md p-2 mb-8 border border-gray-300 rounded shadow-xl bottom-14 ax-w-md" v-model="input"
        placeholder="What is the temperature in the living room?" />
    </form>

    <button @click="stop" :disabled="status === 'awaiting_message'"
