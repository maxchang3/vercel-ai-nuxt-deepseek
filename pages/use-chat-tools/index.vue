    ...
    <div v-for="message in messageList" :key="message.id" class="whitespace-pre-wrap">
      ...
      <template v-for="part in message.parts">
        ...
        <template v-else-if="part.type === 'tool-invocation'">
          <template v-if="part.toolInvocation.toolName === 'askForConfirmation'">
            <template v-if="part.toolInvocation.state === 'call'">
              <div :key="part.toolInvocation.toolCallId" className="text-gray-500">
                {{ part.toolInvocation.args.message }}
                <div className="flex gap-2">
                  <button class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700" @click="
                    addToolResult({
                  ...
                  <button class="px-4 py-2 font-bold text-white bg-red-500 rounded hover:bg-red-700" @click="
                    addToolResult({
                      toolCallId: part.toolInvocation.toolCallId,
                      result: 'No, denied',
                    })
          ...
          <template v-if="part.toolInvocation.toolName === 'getWeatherInformation'">
