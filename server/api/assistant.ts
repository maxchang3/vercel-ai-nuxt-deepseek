...
export default defineLazyEventHandler(async () => {
  ...
  return defineEventHandler(async (event: any) => {
    ...
    const threadId = userThreadId ?? (await deepseek.beta.threads.create({})).id

    // Add a message to the thread
    const createdMessage = await deepseek.beta.threads.messages.create(
      threadId,
      {
        role: 'user',
        content: message,
    ...
    return AssistantResponse(
      ...
      async ({ forwardStream, sendDataMessage }) => {
        ...
        const runStream = deepseek.beta.threads.runs.stream(
          threadId,
        ...
        while (
          ...
          runResult = await forwardStream(
            deepseek.beta.threads.runs.submitToolOutputsStream(
              threadId,
              runResult.id,
              { tool_outputs },
              { signal },
